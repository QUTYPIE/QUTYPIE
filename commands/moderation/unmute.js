...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(!member){
      return message.reply({embeds: [new MessageEmbed().setColor(client.color).setDescription(`${client.emoji.cross} | You didn't mentioned the member whom you want to mute.`)]})
    }
...
async function untimeout(issuer, target, reason) {
  ...
  else if (response === "MEMBER_PERM") return getEmbed(`${client.emoji.cross} | You don't have enough permissions to unmute <@${target.user.id}>.`, client);
  else if (response === "NO_TIMEOUT") return getEmbed(`${client.emoji.cross} | <@${target.user.id}> is not muted only!`, client);
  else return getEmbed(`${client.emoji.cross} | I don't have enough perms to unmute <@${target.user.id}>.`, client);
};
